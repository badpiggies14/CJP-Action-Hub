...
.section-shell{width:100%;max-width:80rem;margin-inline:auto;padding-inline:1rem}
.section-title{font-family:"Arial Narrow",Impact,sans-serif;font-size:clamp(3rem,8vw,4.5rem);font-weight:900;line-height:.92;text-transform:uppercase}
.section-kicker{font-size:.875rem;font-weight:900;letter-spacing:.16em;text-transform:uppercase;color:rgb(var(--stamp))}
.button-primary,.button-secondary,.button-ghost{display:inline-flex;align-items:center;justify-content:center;gap:.5rem;border:2px solid rgb(var(--ink));padding:.75rem 1.25rem;font-size:.875rem;font-weight:900;text-transform:uppercase;box-shadow:4px 4px 0 rgb(var(--ink))}
...
.poster-card{position:relative;overflow:hidden;border:2px solid rgb(var(--ink));background:rgb(var(--newsprint));box-shadow:7px 7px 0 rgb(var(--ink));padding:1.25rem}
.stamp-border{border:2px solid rgb(var(--stamp));outline:1px dashed rgb(var(--stamp)/.65);outline-offset:-8px}
.input-field{width:100%;border:2px solid rgb(var(--ink));background:rgb(var(--newsprint));padding:.75rem 1rem;font-size:.875rem;font-weight:700;color:rgb(var(--ink))}
